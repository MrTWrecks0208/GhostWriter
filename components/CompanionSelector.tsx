...
const CompanionSelector: React.FC<CompanionSelectorProps> = ({ companions, selectedCompanion, onSelect, onClose, onGoToPricing }) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`flex-shrink-0 w-3 h-3 rounded-full ${isLocked ? 'bg-gray-600' : 'bg-accent'}`}></div>
                  <div>
                    <h3 className={`font-semibold text-lg ${isLocked ? 'text-gray-400' : 'text-gray-100'}`}>
                      {companion.name} <span className="text-sm font-normal text-gray-500">({companion.gender})</span>
                    </h3>
                    <p className={`text-sm ${isLocked ? 'text-gray-500' : 'text-gray-400'}`}>{companion.description}</p>
                  </div>
                </div>
                {isLocked && (
